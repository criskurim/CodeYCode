...
class Janela (QMainWindow):
    ...
    #botões
        botaoIniciar = QPushButton('Iniciar', self)
        botaoIniciar.move(250, 250)
        botaoIniciar.resize(80, 80)
        botaoIniciar.setStyleSheet('QPushButton {background-color:#426712; font:bold; font-size:20px}')
        botaoIniciar.clicked.connect(self.botaoIniciar_click)
        botaoSalvar = QPushButton('Salvar', self)
        botaoSalvar.move(450, 250)
        botaoSalvar.resize(80, 80)
        botaoSalvar.setStyleSheet('QPushButton {background-color:#426712; font:bold; font-size:20px}')
        botaoSalvar.clicked.connect(self.mostrar_texto)
    #label
        self.label_1 = QLabel(self)
        #self.label_1.setText("Oi eu sou Bet!")
        self.label_1.move(50, 50)
        self.label_1.setStyleSheet('QLabel {font:bold; font-size:25px; color:"black"}')
        self.label_1.resize(500, 25)
        self.icone_bet = QLabel(self)
        self.icone_bet.move(300, 400)
        self.icone_bet.setPixmap(QtGui.QPixmap('icone.png'))
        self.icone_bet.resize(150, 150)
    #caixa de texto
        self.caixa_texto = QLineEdit(self)
        self.caixa_texto.move(280, 150)
        self.caixa_texto.resize(200, 50)
        self.caixa_texto.setStyleSheet('QLineEdit {font: bold; font-size:25px}')
    #carregamento de janela
        self.CarregarJanela()
    ...
    def botaoIniciar_click(self):
        # início do programa
        bet_inicio = "Olá, eu sou Bet, sua assistente virtual!"
        #self.label_1.setText(bet_inicio)
        sintese_voz(bet_inicio)
        conteudo = agenda_c_bd.lembrete_jogo()
        self.label_1.setText(conteudo)
        #loop para ouvir continuamente
        ouvindo = True
        while ouvindo:
            bet_diz = 'Dê um comando!'
            self.label_1.setText(bet_diz)                                   #solicita que o usuária diga algo
            sintese_voz(bet_diz)
